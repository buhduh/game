...
void Vulkan::drawGUI(
	const std::vector<GUIVertex> vBuffer,
...
void Vulkan::createGPUPipeline() {
	...
	{
		std::string fName("assets/shaders/gui.vert.spv");
	...
	{
		std::string fName("assets/shaders/gui.frag.spv");
		size_t shaderFSize = platform::getFileSize(fName);
		//this might be an issue
		char buffer[shaderFSize];
		size_t loaded = platform::loadFileIntoBuffer(buffer, shaderFSize, fName);
	...
	attributeDescriptions[1].offset = offsetof(GUIVertex, uv);
